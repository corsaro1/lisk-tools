...
while true; do
HEIGHT1=$(curl --connect-timeout 6 -s "http://"$SRV1":8000/api/loader/status/sync"| jq '.height')
HEIGHT2=$(curl --connect-timeout 6 -s "http://"$SRV2":8000/api/loader/status/sync"| jq '.height')
HEIGHT3=$(curl --connect-timeout 6 -s "http://"$SRV3":8000/api/loader/status/sync"| jq '.height')
HEIGHT4=$(curl --connect-timeout 6 -s "http://"$SRV4":8000/api/loader/status/sync"| jq '.height')


FORGE1=$(curl --connect-timeout 6 -s "http://"$SRV1":8000/api/delegates/forging/status?publicKey="$pbk1| jq '.enabled')
FORGE2=$(curl --connect-timeout 6 -s "http://"$SRV2":8000/api/delegates/forging/status?publicKey="$pbk1| jq '.enabled')
FORGE3=$(curl --connect-timeout 6 -s "http://"$SRV3":8000/api/delegates/forging/status?publicKey="$pbk1| jq '.enabled')
FORGE4=$(curl --connect-timeout 6 -s "http://"$SRV4":8000/api/delegates/forging/status?publicKey="$pbk1| jq '.enabled')
...
if ! [[ "$HEIGHT3" =~ ^[0-9]+$ ]]
        then
        HEIGHT3=$(curl --connect-timeout 3 -s "http://"$SRV3":8000/api/loader/status/sync"| jq '.height')
        if ! [[ "$HEIGHT3" =~ ^[0-9]+$ ]]
        then
            echo ""
            echo $SRV3 " " "is off?"
...
if ! [[ "$HEIGHT4" =~ ^[0-9]+$ ]]
        then
        HEIGHT4=$(curl --connect-timeout 3 -s "http://"$SRV4":8000/api/loader/status/sync"| jq '.height')
        if ! [[ "$HEIGHT4" =~ ^[0-9]+$ ]]
        then
            echo ""
            echo $SRV4 " " "is off?"
...
if [ "$FORGE1" = "false" -a "$FORGE2" = "false" -a "$FORGE3" = "false" -a "$FORGE4" = "false" ]
   then
    ...
    curl --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":'"$SECRET"'}' https://"$SRV3":2443/api/delegates/forging/disable
    curl --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":'"$SECRET"'}' https://"$SRV4":2443/api/delegates/forging/disable
    ...
    curl --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":'"$SECRET"'}' https://"$SRV1":2443/api/delegates/forging/disable
    curl --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":'"$SECRET"'}' https://"$SRV2":2443/api/delegates/forging/disable
    curl --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":'"$SECRET"'}' https://"$SRV3":2443/api/delegates/forging/enable
    curl --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":'"$SECRET"'}' https://"$SRV4":2443/api/delegates/forging/disable
    ...
    curl --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":'"$SECRET"'}' https://"$SRV2":2443/api/delegates/forging/disable
    curl --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":'"$SECRET"'}' https://"$SRV3":2443/api/delegates/forging/disable
    curl --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":'"$SECRET"'}' https://"$SRV4":2443/api/delegates/forging/enable
...
if [ "$FORGE1" = "true" -a "$FORGE2" = "true" -o "$FORGE1" = "true" -a "$FORGE3" = "true" -o "$FORGE2" = "true" -a "$FORGE3" = "true" -o "$FORGE1" = "true" -a "$FORGE4" = "true" -o "$FORGE2" = "true" -a "$FORGE4" = "true" -o "$FORGE3" = "true" -a "$FORGE4" = "true" ]
   then
    curl --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":'"$SECRET"'}' https://"$SRV1":2443/api/delegates/forging/enable
    curl --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":'"$SECRET"'}' https://"$SRV2":2443/api/delegates/forging/disable
    curl --connect-timeout 6 -k -H "Content-Type: application/json" -X POST -d '{"secret":'"$SECRET"'}' https://"$SRV3":2443/api/delegates/forging/disable
